...
#%%
#loop Data special
B6_M_loop = np.loadtxt('/public/home/shidetong/wedata/test/Hic/Allilc_loop/orign_loop_cut/end_loop/no_noise_loop/cut/B6_M.csv',dtype = data_type ,skiprows=1 , usecols = (0 , 1 , 2))
B6_P_loop = np.loadtxt('/public/home/shidetong/wedata/test/Hic/Allilc_loop/orign_loop_cut/end_loop/no_noise_loop/cut/B6_P.csv',dtype = data_type ,skiprows=1 , usecols = (0 , 1 , 2))
Cast_M_loop = np.loadtxt('/public/home/shidetong/wedata/test/Hic/Allilc_loop/orign_loop_cut/end_loop/no_noise_loop/cut/Cast_M.csv',dtype = data_type ,skiprows=1 , usecols = (0 , 1 , 2))
Cast_P_loop = np.loadtxt('/public/home/shidetong/wedata/test/Hic/Allilc_loop/orign_loop_cut/end_loop/no_noise_loop/cut/Cast_P.csv',dtype = data_type ,skiprows=1 , usecols = (0 , 1 , 2))
B6_M_loop = np.array(B6_M_loop)
B6_P_loop = np.array(B6_P_loop)
Cast_M_loop = np.array(Cast_M_loop)
Cast_P_loop = np.array(Cast_P_loop)
Loops = {'B6_M_loop':B6_M_loop,
        'B6_P_loop':B6_P_loop,
        'Cast_M_loop':Cast_M_loop,
        'Cast_P_loop':Cast_P_loop}


#%%
Left = 0.2 ; HB = 0.2 ; width = 0.6 ; HH = 0.6  
for cl in ['Cast_M_loop']:
    for c in ['Cast_M']:
        P_Loops = Loops[cl]
        apa = []                    
        for g in chros:
        ...
        sc = ax.imshow(avg, cmap = my_cmap, aspect = 'auto', interpolation = 'none',
                       extent = (0, len(avg), 0, len(avg)), vmin = 0 , vmax =5, origin = 'lower')
        ax.set_xticks([0 ,5,10])
        ax.set_xticklabels(['-100K' , 'Loop'  , '100K'])
        ax.set_yticks([0 , 5 ,10 ])
        ax.set_yticklabels(['-100K' , 'Loop'  , '100K'])
        ax.set_xlabel('CBF1_Cast', fontsize = 15)
